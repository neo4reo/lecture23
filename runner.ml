...
(* List the files that are prepended to each program here *)
let library_files = List.map ((^) "input/") []
...
let parse_string (name : string) (s : string) : Expr.program =
  let s'     = library_string ^ s in
  let lexbuf = Lexing.from_string s' in
  parse name lexbuf

let parse_file (name : string) (input_file : in_channel) : Expr.program =
  let s'     = library_string ^ input_all input_file in
  let lexbuf = Lexing.from_string s' in
  parse name lexbuf
